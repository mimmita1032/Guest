...
void UGEquipSlotWidget::RefreshSlotUI(UGInventoryItemWidget* ItemWidget)
{
...
bool UGEquipSlotWidget::NativeOnDrop(const FGeometry& InGeometry, const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
	...
	UGItemInstance* ItemInst = InvComp->GetItemByHandle(DragOp->DraggedHandle);
	if (!ItemInst) return false;

	const UGItemDefinition* ItemDef = ItemInst->GetItemDef();
	if (!ItemDef) return false;

	// 1. 태그 검사: 이 슬롯에 허용된 부위인지 확인
	if (AllowedItemTag.IsValid() && !ItemDef->HasTag(AllowedItemTag))
	{
		return false; 
	}

	// 2. 장착 실행
	if (InvComp->EquipItem(DragOp->DraggedHandle, EquipSlotType))
	{
		// 장착 사운드 재생
