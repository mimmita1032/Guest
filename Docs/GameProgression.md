...
### `L_TavernMain` 구조 (2026-09-04 재구축)

기존 주점은 지하에 작은 방 여럿으로 쪼개져 있어 좁았다. 액터 421개가 전부 기본 이름이라
어디가 무엇인지 읽히지도 않았다. 걷어내고 **지상 2층**으로 다시 지었다. 액터 이름은 `Tav_` 접두어.

| | 범위 | 층고 |
|---|---|---|
| 1층 홀 | `X 0~2400`, `Y 0~1600` (24×16m) | 450 |
| 아트리움 (2층까지 뚫림) | `X 680~1720`, `Y 160~840` | 830 |
| 2층 | 같은 footprint, 아트리움·계단실 제외 | 350 |

- **정문** 남쪽 `X 1050~1350`, **뒷문** 북쪽 `X 250~500`
- **계단** 동쪽 `X 2050~2350`, `Y 300~840`을 18단(디딤 25cm)으로 오름. 2층 바닥에 계단실 구멍(`X 2000~2430`, `Y 250~900`)
- **2층**: 침실(`X 30~1100`) / 작업실(`X 1130~1700`), 칸막이는 `Y 1000~1030`. 그 남쪽 `Y 840~1000`이 방 앞 복도
- **게임플레이 지점**: `PlayerStart` (1200, 190) 홀을 향함 / `Reach_Home` 트리거 (1200, 430) /
  화분 자리 `GItemPlacementPoint` (62, 450) — 서쪽 창턱, 스미스 코너석 바로 옆 / `BP_Smith` (410, 640)
