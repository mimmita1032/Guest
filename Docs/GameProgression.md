...
### ⬜ 검증 대기 — 피사체 인식 인게임 확인 (2026-08-17)

**코드·배치·문서는 끝났고 `dev`에 머지됐다 (PR #107). 남은 건 실제로 찍어보는 것뿐이다.**

지금까지의 판정 결과는 **실제 셔터가 아니라 같은 투영식으로 계산한 예측**이다.
최종 확정된 상자·임계값으로 인게임 촬영은 아직 한 번도 하지 않았다.

`L_Residential_01`에서 세 장을 찍고 Output Log를 본다:

| # | 어디서 | 무엇을 | 기대 |
|---|---|---|---|
| 1 | 골목 아래 | 언덕 위 마을 | `피사체 판정: Photo_House` |
| 2 | 언덕 정상 | 아파트 쪽(+X) | `피사체 판정: Photo_Apartment` |
| 3 | 언덕 정상 | 하늘 | **아무 줄도 안 나온다** |

**3번이 이번 작업의 핵심이다.** 예전에는 하늘을 찍어도 `Photo_House`가 기록돼 퀘스트가
진행됐다. 예측상 정상에서 하늘을 향하면 두 피사체 모두 0이 나온다.

- 결과가 어긋나면 로그의 `점유율 %` / `점수`를 보고 `MinScreenCoverage`(0.04)나
  `MinVisibleRatio`(0.25)를 조정한다. 상자 좌표는 「2-2. 피사체 컴포넌트 배치」에 있다
- 디카를 열 때 `CaptureScene: ... major inefficiency` 경고가 **사라졌는지** 같이 확인
- 세 장 다 맞으면 `BP_GuestCharacter`의 카메라 컴포넌트에서
  `SubjectResolution`을 **`FrameOnly`** 로 옮긴다. 그래야 좌표 판정 폴백이 끊긴다
  (그 전까지는 하늘을 찍어도 좌표값이 대신 들어간다 — 즉 **지금은 아직 안 고쳐진 상태다**)

그 다음은 나머지 레벨(랜드마크 3종)에 피사체를 배치하는 일이다.
...
2. **피사체 인식 (프레임 기반)** — 코드·배치 완료, `dev` 머지됨 (PR #107).
   ← **인게임 검증만 남았다. 「TODO ⬜ 검증 대기」 참고.**
   검증 전까지는 `SubjectResolution`이 `FrameThenPlace`라 **아직 좌표 폴백이 살아 있다**
