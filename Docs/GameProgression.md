...
> ⚠️ **랜드마크 3행의 `RequiredStoryProgress`가 현재 `0`이다.** 테스트를 막지 않으려고 열어둔 것이고,
> `Q_Smith_002`를 만들 때 **`5`로 올려야 한다.** 안 그러면 Stage 0에서도 랜드마크로 갈 수 있다.
...
- ⬜ **(인게임 검증 후)** `BP_GuestCharacter`의 카메라 컴포넌트에서 `SubjectResolution`을
...
| Traditional | `Photo_Traditional` | `2189, 0, 1320` | `2111, 2550, 1120` | `40_Hall`만 |
...
>
> **마커 세 곳 중 한 곳만 통과하면 된다 (2026-08-27 확정).** 그래서 문이 본체를 가리는
> `PHOTO_OutsideGate`가 `MinVisibleRatio`(기본 0.25)에 걸려도 그대로 둔다.
> 축·사선 두 구도 중 하나만 나오면 촬영이 성립한다.
