...
1. **사진 비율·해상도 정리 (고도화 때)** — 촬영본이 `1920x1080`인데 썸네일 브러시는
   `300x180`이라 상세 패널에서 사진이 찌그러진다. 또 한 장이 약 8MB(BGRA)라
   장수가 쌓이면 메모리와 세이브 용량이 그대로 곱해진다. 촬영 해상도를 낮추거나
   썸네일용 축소본을 따로 만드는 쪽으로 정리할 것
2. **`L_Residential_01` 촬영 구역 제작** — Stage 2~4를 완주시키려면 실제로 찍을 풍경이 필요하다.
   현재 좌표(`Residential_2010`)와 `PhotoSubjectID`(`Photo_House`)는 이미 있다
3. **UI 텍스트 넘침 정리** — 전체화면에서 목표 문구·대사·버튼 라벨이 화면 밖으로 잘린다.
   UMG TextBlock에 줄바꿈 기준 폭이 없어서다. 브랜치 `feat/packaging-test`에서 진행 중
4. **패키징 테스트 환경** — `GameDefaultMap`·`bCookAll`은 잡았다. `Packaged/`를 `.gitignore`에
   추가하고 한 번 패키징해볼 것
5. **Stage 5~8** — 스미스 체인 나머지 (`Q_Smith_002`, `Q_Smith_003`)
