...
| 3 | 현실 · 주택가 (`L_Residential_01`) | 평범한 건물 촬영. **맹견이 잘못된 길을 막는다** | 주택가 풍경 촬영, 맹견은 따돌린다 | 사진 확보 후 귀가 | 구현됨 (맹견 배치만 남음) |
| 4 | 주점 (`L_TavernMain`) | 사진 전달. 스미스가 "영감이 안 온다"며 보수 지불 | 스미스와 대화 | `Q_Smith_001` 완료 | 구현됨 |
| 5 | 주점 (`L_TavernMain`) | 스미스 재방문. 독특한 건물 의뢰 | 대화 | `Q_Smith_002` 수락 | 구현됨 |
| 6 | 현실 · 랜드마크 3종 | 독특한 건물 촬영 | 미술관/전통건축/스카이스파이어 촬영 | **3장을 다 전달** → 스미스가 스카이스파이어를 택함 | 구현됨 |
| 7 | 현실 · 스카이스파이어 (`L_Landmark_SkySpire_01`) | 추가 의뢰 — 입체 모형 수거 | 내부 전망대에서 모형 획득 | 모형 들고 귀가 | 구현됨 |
| 8 | 주점 (`L_TavernMain`) | 모형 전달. 스미스 보수 지불 후 한동안 이탈 | 스미스와 대화 | `Q_Smith_003` 완료 | 구현됨 |
| 9 | 중앙 도시 (`L_CitySquare`) | **엔딩** — 스미스가 완성된 홀로그램 설계도를 보여주며 감사 인사 | 관람 | 데모 종료 화면 | 구현됨 |
...
- ✅ **(에디터)** `DA_Item_Photo` 생성 + 카메라 컴포넌트에 지정 — 「에디터에서 해야 할 일 · 1」에서 실측 확인 완료
...
- ✅ 데모 종료 화면 — `WBP_DemoEnd` / `GuestDemoEndWidget`, 나레이션 시스템 재사용으로 구현 완료
...
- **구현 현황**:
  ...
  - ✅ (에디터) 나레이션 애셋 본문 작성 — `DA_Narration_Tavern_001` 세 장으로 채움 (2026-09-03)
...
> **현황 (2026-09-03).** AI는 PR #64·#81·#102·#105·#111로 완성돼 있다 —
> 시야 감지, 추격, 공격, Patrol이 모두 동작한다.
> 남은 것은 **NavMesh와 배치**뿐이며 둘 다 에디터 작업이다.
> 죽었을 때의 처리(주점 복귀)는 PR #118로 들어갔다.
...
- **구현 현황**:
  - ✅ `Q_Smith_002` 퀘스트·대화 구현 (2026-09-02) — 인게임 완주 확인
  - ✅ (에디터) 랜드마크 3개 맵 블록아웃 — SkySpire(PR #104), Museum·Traditional(이번 작업)
  - ✅ (에디터) 랜드마크 3개 맵 촬영 대상 정비 — 「피사체 인식」절 참고, 2026-08-27 실측 확인 완료
  - ~~사진 여러 장 중 하나를 NPC가 고르는 대화 분기~~ ✅ 불필요로 확정 — 3장을 다 주고 스미스가 택한다
- **구현 상태**: 구현됨 (2026-09-02)

### 대화 선택지 조건 문법

`FDialogueChoice::ConditionID`에 쓴다.

| 문법 | 뜻 |
|---|---|
| `Q_Smith_001` | 진행 중 |
| `Q_Smith_001.Step02` | 진행 중 + 현재 단계 일치 |
| `Q_Smith_001.Completed` | 완료함 |
| `!조건` | 부정 |
| `조건A&조건B` | 둘 다 만족 |

수락 선택지는 조건 하나로 안 된다. "1번을 끝냈다"만 걸면 2번을 수락한 뒤에도 계속 뜨고,
`IsQuestActive`는 완료된 퀘스트에도 `false`를 주므로 부정 하나로도 안 된다. 그래서
`Q_Smith_001.Completed&!Q_Smith_002&!Q_Smith_002.Completed`처럼 세 개를 잇는다.

### Talk 목표는 대화 노드가 발화한다

`FDialogueNode::TalkObjectiveID`에 넣는다. **NPC 속성에 두면 안 된다.**

같은 NPC라도 트리거로 다가가면 바 대화(`OpenBarDialogue`), `E`를 누르면 NPC 대화
(`OpenNPCDialogue`)로 열리는데, 목표를 NPC에 두면 **바 대화 경로에서만 발화한다.**
실제로 스미스 등 뒤에서 `E`로 말을 걸면 대화는 정상인데 퀘스트가 끝나지 않았다.
노드가 들면 공통 부모(`UGuestDialogueWidgetBase`)가 처리하므로 어느 경로든 같다.
...
- **구현 현황**:
  ...
  - ✅ `DA_Item_SkySpireModel` (`ITEM_SKY_001`) — 2×2, `SM_SkySpireModel`
  - ✅ `L_Landmark_SkySpire_01` 모형 배치 — 전망대는 블록아웃(PR #104)에 이미 있었다
  - ✅ `Q_Smith_003` 퀘스트·대화 (2026-09-02) — 인게임 확인
- **구현 상태**: 구현됨 (2026-09-02)

### 모형 메시는 타워를 병합해 만들었다

`Sky/Tower` 12조각을 **0.002로 축소한 복제본으로 만들어 병합**했다(`SM_SkySpireModel`, 14×14×72cm).
실루엣이 실제 탑과 같아 스미스의 "위로 갈수록 조금씩 돌아가는군" 대사가 손에 든 물건과 맞는다.

> ⚠️ **병합할 때 `merge_physics_data`를 켜야 한다.** 끄고 병합하면 콜리전 형상이 0개인
> 메시가 나오고, 컴포넌트 설정이 멀쩡해도 트레이스가 그냥 통과해 상호작용이 안 된다.
>
> ⚠️ **메시 자체를 모형 크기로 만들어야 한다.** 액터 스케일로만 줄이면 버리거나 다시
> 스폰될 때 원본 크기(362m)로 나온다. `build_scale3d`는 파이썬에서 리빌드가 걸리지
> 않아 바운즈에 반영되지 않았다 — 축소 복제본을 만들어 병합하는 쪽이 확실하다.

### 전망대까지는 텔레포트로 간다

엘리베이터를 실제로 만들지 않기로 했다(2026-09-02 확정). 타워가 362m라 올라가는 연출을
만드는 비용이 데모 값어치에 비해 크다.

`AGTeleportPoint`를 새로 만들어 1층 문(`TP_Lobby`)과 전망대 문(`TP_Deck`)을 잇는다.
`AGDoorLevelTransition`은 레벨을 갈아끼우므로 한 레벨 안의 이동에는 쓸 수 없다.

### 뒤쪽 스테이지 확인은 DebugSkipToQuest로

```
DebugSkipToQuest Q_Smith_003
```

`RequiredQuestID`를 거슬러 올라가 선행 퀘스트를 전부 완료 처리한 뒤 목표를 수락한다.

> ⚠️ **`DebugSetStoryProgress`로 진행도만 올려서는 수락되지 않는다.**
> `AcceptQuest`는 선행 퀘스트가 **실제 완료 장부에 있는지**를 본다.
...
- **결말**: 데모 종료 화면
- **구현 현황**:
  - ✅ 선행작업 E (데모 엔딩 화면) — `WBP_DemoEnd` (2026-09-03)
  - ✅ `CitySquare_2030` 좌표 행 (2030 / 21, 진행도 5 이상)
  - ✅ 엔딩 나레이션 `DA_Narration_Ending` + `AGNarrationTrigger` 배치
  - ✅ (에디터) 스미스 배치 — `SMITH_Stand` 자리, `RequiredStoryProgress = 5`
...
- **구현 상태**: 구현됨 (2026-09-03)

### 엔딩은 도착이 계기다

Stage 9에는 퀘스트가 없다. `Q_Smith_003`이 완료되면 진행도가 5가 되고 거기서 끝이며,
**중앙 도시에 도착한 것 자체가 엔딩의 조건**이다.

그래서 `AGNarrationTrigger`를 새로 만들었다 — 레벨에 도착하면 나레이션을 재생하는 액터다.
`RequiredStoryProgress`로 게이팅하므로 진행도가 모자라면 아무 일도 하지 않는다.

나레이션이 걷히면 데모 종료 화면으로 넘어간다. 이 전환은 `UGNarrationDataAsset`의
**`bShowDemoEndOnFinish`** 로 켠다 — 엔딩 나레이션에만 켜고 나머지는 지금처럼 그냥 닫힌다.

> **연도를 2030으로 둔 이유.** 스미스는 모형을 받고 "한동안은 못 올 것 같소,
> 지어야 할 게 생겼거든"이라고 말한다. 사진을 찍던 2026년 그대로면 그 사이가 없어진다.
> 설계도를 완성하고 실제로 짓는 데 걸린 시간이 좌표에 드러나야 한다.

> ✅ **한 번만 재생되게 고쳤다 (2026-09-03).** `GuestGameInstance::TryMarkPlayedOnce` —
> 배치 지점(`PlacedPointIDs`)과 같은 방식으로 `PlayedOnceIDs`에 기록해 중앙 도시에
> 다시 들어가도 재생되지 않는다. `AGNarrationTrigger::bPlayOnce`로 켠다.
...
> 랜드마크 3행의 `RequiredStoryProgress`는 **`3`** 이다 (2026-09-02 정정).
>
> 처음에 진행 단계표의 「Stage 5」를 그대로 진행도 5로 옮겨 적었는데 **스테이지 번호와
> 진행도 값은 1:1이 아니다.** 실제 진행도는 `Q_Field_001` 완료 → 1, 나레이션 → 2,
> `Q_Smith_001` 완료 → 3으로 오른다. 5로 두면 첫 의뢰를 끝내도 랜드마크에 갈 수 없다.
> 진행도를 손댈 때는 표의 스테이지 번호가 아니라 `DT_QuestData`의 실제 값을 볼 것.
...
4. ~~**Stage 2~4** — 스미스 첫 방문~~ ✅ 완료
5. ~~**Stage 5~8** — 스미스 체인 나머지~~ ✅ 완료
6. ~~**Stage 9 + E** — 엔딩과 데모 종료 화면~~ ✅ 완료

**Stage 0~9 전 구간 코드·콘텐츠 완주 (2026-09-03 기준).** 남은 건 전부 에디터/아트 마감뿐이다 —
「에디터에서 해야 할 일」과 노션 데모 작업 보드 참고. 이 표는 더 갱신할 코드 순서가 없다.
...
4. **UI 텍스트 넘침 정리** — 전체화면에서 목표 문구·대사·버튼 라벨이 화면 밖으로 잘리던 것을
   코드에서 잡았다. `UGuestBlueprintLibrary::ApplyAutoWrap()`을 만들어 대사·응답 라벨·선택지·
   퀘스트 제목/설명/목표·나레이션 본문·사진 장소명 위젯의 `NativeOnInitialized`에서 호출한다.
   WBP 설정과 무관하게 자동 줄바꿈이 보장된다. 부모가 폭을 안 잡아주는 배치를 대비해 위젯마다
   `...WrapTextAt`(px, 기본 0 = 부모 폭 기준) 프로퍼티를 열어뒀다 — 실기 확인 후 필요한 곳만 채울 것.
   탭 버튼 라벨은 접으면 탭 크기가 흔들려 제외했다
5. ~~**패키징 테스트 환경**~~ ✅ 완료 (2026-09-05, PR #126·#127). 아래 「패키징」절 참고
...
|---|---|---|---|
| `L_Field_01` | 완료 | — | — |
| `L_TavernMain` | 목조 3층으로 재구축 (2026-09-05, PR #123) | 402 | — |
...
### `L_TavernMain` 구조 (2026-09-05 재구축, PR #123)

원래 주점은 지하에 작은 방 여럿으로 쪼개져 좁았고, 그 뒤 한 번 넓은 2층으로 다시 지었더니
이번엔 중세 골목의 허름한 주점이 아니라 프랜차이즈 매장처럼 읽혔다. **좁고 높은 목조 3층**으로
다시 지었다. 액터 이름은 `Tav_` 접두어.

| | 범위 |
|---|---|
| 건물 footprint | `X 620~1300`, `Y 0~1300` (6.8×13m) |
| 실내 | `X 642~1278`, `Y 22~1278` |
| 층고 | 285 (각 층). 1층 바닥 0 / 2층 313 / 3층 626 |

- **용도**: 1층 홀 + 다찌, 2층 침실, 3층 창고
- **정문** 남쪽 `X 1120~1280`. 서쪽 벽 `Y 90~250`에 창(화분 자리)
- **계단** 동쪽 `X 1100~1268`을 11단씩 두 번. 층 바닥에 계단실 구멍 `X 1080~1322`, `Y 300~795`
- **다찌**는 동쪽 벽에 붙은 일자 카운터(`X 880~1278`, `Y 850~960`)다. 주인장이 드나드는 통로는
  **서쪽 벽 쪽** `X 642~880`. 카운터를 동쪽 벽까지 붙이지 않으면 계단(`X 1100~1268`)과 X가 겹쳐
  통로가 아예 막힌다 — 처음에 이걸로 한 번 막혔다. 카운터 뒤 주인장 공간은 `Y 960~1240`
- **게임플레이 지점**: `PlayerStart` (1200, 120, 88) / `GQuestReachTrigger` (1000, 300, 90) /
  화분 자리 `GItemPlacementPoint` (668, 170, 130) — 서쪽 창턱 / `BP_Smith` (810, 270, 88)
- 건물 주위로 같은 모양의 이웃집(`Nb*`)이 골목을 이룬다. 문은 전부 닫혀 있다

> **`BP_Smith`의 피벗은 캡슐 중심(허리)이다.** Z=0에 놓으면 바닥에 절반 파묻힌다. Z=88로 둘 것.

> **주점 바닥과 랜드스케이프가 둘 다 Z=0이라 겹쳐 있었다.** 실내에서 나무 바닥 대신 지면이
> 비쳐 보인다. 지면을 −20으로 내려 해결했으니, 이 맵에서 지면 높이를 다시 만질 때 주의할 것.

> `GItemPlacementPoint`의 `PlacementID`가 비어 있었다. 배치 기록이 이 ID로 저장되므로 비어
> 있으면 레벨 재입장 시 복원이 어긋난다. `Tavern_FlowerPot`으로 채웠다.

> 1층 실내 배치(가구·조명)는 PR #123 머지 후 에디터에서 따로 손봤다. 위 수치는 건물 골조와
> 게임플레이 지점 기준이다.
...
한 번 돌려 기준선을 잡고 설정을 좁혔다. **빌드·쿡·스테이징·아카이브까지 통과하고, 패키징본을
실행해 들판 진입 → 주점 이동 → 디지캠·대화·나레이션 위젯까지 동작을 확인했다.**
...
기준선 경고 10,643개 중 10,638개가 **쓰지 않는 마켓 에셋 팩 하나**(`IdaFaber/Demo/Animations`)에서
나왔다. `bCookAll`이 켜져 있어 프로젝트 전체를 굽고 있었다.

- `bCookAll` → `False`, `bSkipEditorContent` → `True`
- `MapsToCook`에 데모 맵 7개를 명시. `TestLevel` 아래 개인 테스트 맵은 굽지 않는다
- `DirectoriesToAlwaysCook`에 `/Game/UI` `/Core` `/Data` `/Items` `/Skills` `/GAS` `/WwiseAudio` 추가

> 마지막 항목이 필요한 이유는 **이 폴더들이 맵에서 하드 참조되지 않기 때문**이다.
> `GuestUISettings`의 위젯 매핑, `GlobalDefaultGameMode`, 나레이션·퀘스트 데이터는 설정과
> 소프트 경로로만 닿는다. 빠지면 패키징본에서 위젯이 안 뜬다. 맵을 늘릴 때 함께 볼 것.

> 기존 `MapsToCook`이 `/Game/maps/Test`를 가리키고 있었는데 **그런 경로는 없다**
> (실제로는 `/Game/maps/TestLevel/Test`). 쿡마다 경고가 찍히고 있었다.

**남은 문제:** 패키징본에 **Wwise 사운드뱅크가 하나도 들어가지 않는다**(`.bnk`/`.wem` 0개,
실행 로그에 `Audio Device Manager Initialization Failed!`). 이 설정 변경 탓이 아니라
기준선에도 0개였다 — 원래부터 그렇다.
